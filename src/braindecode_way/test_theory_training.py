...
set_random_seeds(seed=20200220, cuda=(device == 'cuda'))
...
all_labels = [label for _, label, _ in windows_dataset]
counter = Counter(all_labels)
total = sum(counter.values())

print("📊 Verteilung der Fensterlabels:")
for cls in sorted(counter):
    count = counter[cls]
    print(f"  Klasse {cls}: {count} Fenster ({count / total:.1%})")
...
plt.tight_layout()
plt.savefig(os.path.join(log_dir, 'training_subject8.png'))
plt.close()
...
fig_cm.savefig(os.path.join(log_dir, 'confmat_subject8.png'))
plt.close(fig_cm)
