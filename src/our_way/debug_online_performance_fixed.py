# -*- coding: utf-8 -*-
"""
Fixed debug script that uses braindecode's exact filtering method.
This should resolve the preprocessing mismatch issue.
"""
...
def load_offline_data():
    ...
    # Exact training preprocessing - using init_block_size=1000 as in training
    preprocessors = [
        ...
        Preprocessor(exponential_moving_standardize, factor_new=1e-3, init_block_size=1000)  # FIXED: Use 1000 as in training
    ]
...
def simulate_online_preprocessing_fixed(test_set, dataset):
    """Simulate online preprocessing using braindecode's exact methods."""
    print("Simulating online preprocessing with braindecode methods...")
    ...
    # Initialize online standardizer with correct parameters
    standardizer = OnlineExponentialStandardizer(
        n_channels=len(included_channels),
        factor_new=1e-3,
        init_block_size=1000  # FIXED: Use 1000 as in training
    )
    ...
    # Process windows using braindecode's exact filtering
    online_results = []
    input_window_samples = 250
    ...
    for i in range(len(test_set)):
        ...
        # Apply online preprocessing using braindecode's methods
        # 1. Scale to microvolts
        scaled_window = raw_window * 1e6

        # 2. Filter using braindecode's exact method
        # Create a temporary MNE Raw object for filtering
        temp_raw = mne.io.RawArray(scaled_window, mne.create_info(
            ch_names=[f'CH{i}' for i in range(scaled_window.shape[0])],
            sfreq=125,
            ch_types=['eeg'] * scaled_window.shape[0]
        ))

        # Apply braindecode's filter (same as Preprocessor('filter', l_freq=4, h_freq=38))
        temp_raw.filter(l_freq=4, h_freq=38, method='iir', picks=None, verbose=False)
        filtered_window = temp_raw.get_data()

        # 3. Standardize using braindecode's exact method
        if standardizer.initialized:
            # Use braindecode's exponential_moving_standardize function
...
def create_visualizations(offline_windows, online_windows, differences, correlations):
    ...
    save_dir = os.path.join(project_root, 'log', 'debug_online_fixed')
...
def main():
    ...
    print("Debugging Online Performance Issues (FIXED VERSION)")
    print("="*60)
    ...
    # Simulate online preprocessing with braindecode methods
    online_windows = simulate_online_preprocessing_fixed(test_set, dataset)
    ...
    print("\n✅ Fixed debugging completed!")
    print("Check the visualizations in log/debug_online_fixed/ for insights.")
